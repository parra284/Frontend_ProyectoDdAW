...
const InventoryDashboard = () => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      setNetworkStatus(navigator.onLine ? 'server-error' : 'offline');
    } finally {
      setLoading(false);
    }
  }, []);

  // Initiate product deletion process
  ...
  const handleDelete = async (reason) => {
    ...
    try {
      ...
      showNotification(`Product "${productToDelete.name}" removed successfully!`, 'success', 3000);
  ...
  useEffect(() => {
    ...
    if (autoRefresh && refreshInterval > 0) {
      intervalId = setInterval(() => {
        if (navigator.onLine) {
          console.log(`Auto-refreshing inventory data (every ${refreshInterval}s)`);
          fetchProducts();
        }
